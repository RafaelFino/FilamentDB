...
def agent_instructions():
    ...
    filament_key = request.args.get("filament_key", "").strip()
    if not filament_key:
        return _json_error("filament_key is required", 400, "filament_key")
    ...
    system_prompt = (
        ...
        "Sua tarefa é encontrar ofertas reais e atuais de filamentos no Brasil. "
        "Use as ferramentas disponíveis, verifique diretamente as páginas das ofertas "
        "e publique somente dados que possam ser comprovados. Não invente fatos. "
        "Ao publicar uma oferta, use exatamente o filament_key fornecido. "
        "Se não encontrar uma oferta confiável, não publique nenhuma."
    )
    user_prompt = (
        f"Pesquise ofertas atuais para o filament_key exato: {filament_key}.\n\n"
        "Execute uma pesquisa completa nas fontes configuradas. Preserve todas as ofertas "
        "verificáveis que correspondam ao fabricante, material e linha/modelo representados "
        "pela chave. Para cada oferta válida, use submit_offer.\n\n"
        "Regras obrigatórias:\n- " + "\n- ".join(rules)
    )
    return jsonify({
        "ok": True,
