...
DB_PATH = config.database_path("filament.db")
...
def list_process_profiles():
    ...
    rows = conn.execute(
        ...
        SELECT
            ...
            pp.support_critical_regions_only,
            pp.brim_width,
            pp.brim_object_gap,
            pp.ironing_type,
            pp.seam_position,
            pp.enable_prime_tower,
            pp.prime_tower_width,
            pp.prime_tower_brim_width,
            pp.flush_into_infill,
            pp.flush_into_support,
            pp.flush_multiplier,
            pp.printer_model,
            pp.nozzle_size,
            pp.base_id,
        ...
        ORDER BY pp.nozzle_size ASC, m.name, pp.layer_height ASC, pp.profile_type, pp.profile_name
        """
    ).fetchall()
    conn.close()
    return rows_to_dicts(rows)
...
def build_process_tree():
    ...
    for row in profile_rows:
        ...
        tree[material]["profiles"].append({
            ...
            "enable_support":               bool(row["enable_support"]) if row["enable_support"] is not None else None,
            "support_type":                 row["support_type"],
            "support_on_build_plate_only":  bool(row["support_on_build_plate_only"]) if row["support_on_build_plate_only"] is not None else None,
            ...
            "support_critical_regions_only": bool(row["support_critical_regions_only"]) if row["support_critical_regions_only"] is not None else None,
