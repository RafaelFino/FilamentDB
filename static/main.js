...
function renderPriceOffer(offer) {
    ...
    return `<div class="price-offer"><div><div class="price-store">${priceEsc(offer.store)}</div>${color}<div class="price-title" title="${priceEsc(offer.title)}">${priceEsc(offer.title || 'Oferta')}</div><a class="price-offer-link" href="${priceEsc(offer.url)}" target="_blank" rel="noopener">Abrir oferta ↗</a></div><div class="price-offer-price">${priceMoney(total)}${shipping}</div></div>`;
}
function renderPrices() {
    ...
    const rows = items.map(x => {
        ...
        return `${groupHtml}<div class="price-report-row"><div><div class="price-filament-name">${priceEsc(x.commercial_name || x.profile_name)}</div><div class="price-filament-meta">${priceEsc(x.filament_key || x.profile_name || '')} · ${priceEsc(x.profile_name || '')}${x.line ? ` · ${priceEsc(x.line)}` : ''}${x.line_finish ? ` · ${priceEsc(x.line_finish)}` : ''}</div></div><div class="price-number">${priceMoney(x.best_price)}</div><div class="price-number">${priceMoney(x.median_price)}</div><div class="price-number">${priceMoney(x.min_price)}</div><div class="price-opportunity">${discount}</div><div class="price-offers">${offers}</div></div>`;
    }).join('');
