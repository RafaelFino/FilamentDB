...
// ═══════════════════════════════════════════════════════════════════════════════
\n// ─── Prices view ──────────────────────────────────────────────────────────────\nlet priceData=null;let priceLoaded=false;\nconst priceGrid=document.getElementById('price-grid');const priceMaterial=document.getElementById('price-material');const priceManufacturer=document.getElementById('price-manufacturer');const priceSort=document.getElementById('price-sort');\nfunction priceMoney(v){return v==null?'—':Number(v).toLocaleString('pt-BR',{style:'currency',currency:'BRL'});}function priceEsc(s){return String(s??'').replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]));}\nfunction populatePriceFilters(){const mats=[...new Set(priceData.items.map(x=>x.material_name))].sort();const mfrs=[...new Set(priceData.items.map(x=>x.manufacturer_name))].sort();priceMaterial.innerHTML='<option value="">Material: Todos</option>'+mats.map(x=>`<option>${priceEsc(x)}</option>`).join('');priceManufacturer.innerHTML='<option value="">Fabricante: Todos</option>'+mfrs.map(x=>`<option>${priceEsc(x)}</option>`).join('');}\nfunction renderPrices(){if(!priceData)return;let items=[...priceData.items];if(priceMaterial.value)items=items.filter(x=>x.material_name===priceMaterial.value);if(priceManufacturer.value)items=items.filter(x=>x.manufacturer_name===priceManufacturer.value);if(priceSort.value==='price')items.sort((a,b)=>a.best_price-b.best_price);else if(priceSort.value==='name')items.sort((a,b)=>(a.commercial_name||'').localeCompare(b.commercial_name||''));else items.sort((a,b)=>b.discount_pct-a.discount_pct||a.best_price-b.best_price);document.getElementById('price-tracked').textContent=priceData.summary.tracked_count;document.getElementById('price-priced').textContent=priceData.summary.priced_count;document.getElementById('price-offers').textContent=priceData.summary.offer_count;document.getElementById('price-best').textContent=items[0]?priceMoney(items[0].best_price):'—';if(!items.length){priceGrid.innerHTML='<div class="empty">Nenhuma oferta encontrada para os filtros.</div>';return;}priceGrid.innerHTML=items.map(x=>`<article class="price-card"><div class="price-card-head"><div class="price-card-title">${priceEsc(x.commercial_name||x.profile_name)}</div><div class="price-card-sub">${priceEsc(x.material_name)} · ${priceEsc(x.manufacturer_name)}${x.line?' · '+priceEsc(x.line):''}</div></div><div class="price-card-body"><div class="price-best"><div><div class="price-value">${priceMoney(x.best_price)}</div><div class="price-unit">melhor preço observado</div></div><div class="price-opportunity">${x.discount_pct>0?'↓ '+x.discount_pct.toFixed(0)+'% vs. histórico':'preço de referência'}</div></div><div class="price-meta"><div><span class="lbl">Loja</span><span class="val">${priceEsc(x.best_store)}</span></div><div><span class="lbl">Mediana</span><span class="val">${priceMoney(x.median_price)}</span></div><div><span class="lbl">Mínimo histórico</span><span class="val">${priceMoney(x.min_price)}</span></div><div><span class="lbl">Ofertas</span><span class="val">${x.offer_count}</span></div></div><div class="price-actions"><a class="btn btn-primary" href="${priceEsc(x.best_url)}" target="_blank" rel="noopener">Ver melhor oferta ↗</a><button class="btn btn-ghost price-history-btn" data-id="${x.id}">Histórico</button></div><div class="price-detail" id="price-detail-${x.id}" style="display:none"></div></div></article>`).join('');priceGrid.querySelectorAll('.price-history-btn').forEach(btn=>btn.addEventListener('click',()=>togglePriceHistory(Number(btn.dataset.id))));}\nasync function togglePriceHistory(id){const el=document.getElementById(`price-detail-${id}`);if(!el)return;if(el.dataset.loaded){el.style.display=el.style.display==='none'?'block':'none';return;}el.innerHTML='<div class="muted-txt">Carregando histórico...</div>';el.style.display='block';try{const r=await fetch(`/api/prices/${id}/history`);const rows=await r.json();el.innerHTML=rows.length?rows.map(x=>`<div class="price-history-row"><span>${new Date(x.collected_at).toLocaleDateString('pt-BR')} · ${priceEsc(x.store)}</span><strong>${priceMoney(x.price)}</strong></div>`).join(''):'<div class="muted-txt">Sem histórico.</div>';el.dataset.loaded='1';}catch(e){el.innerHTML='<div class="muted-txt">Erro ao carregar histórico.</div>';}}\nasync function loadPrices(){if(priceLoaded)return;try{const r=await fetch('/api/prices');if(!r.ok)throw new Error(`HTTP ${r.status}`);priceData=await r.json();priceLoaded=true;populatePriceFilters();renderPrices();}catch(e){priceGrid.innerHTML='<div class="empty">Não foi possível carregar o histórico de preços.</div>';console.error('Failed to load prices',e);}}\n[priceMaterial,priceManufacturer,priceSort].forEach(el=>el?.addEventListener('change',renderPrices));\n
