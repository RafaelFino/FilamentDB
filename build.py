...
# Banco principal: caminho resolvido pela configuração central.
# DB_PATH vazio/ausente cai no default de config.py (data/filament.db).
...
def make_filament_key(material, manufacturer, model):
    return "|".join(normalize_key_part(x) for x in (material, manufacturer, model))
...
def create_schema():
    ...
    if Path(DB_PATH).exists():
        try:
            ...
            try:
                rows = old.execute("""SELECT fp.id, fp.profile_name, fp.commercial_name, fp.line, m.name AS material, mf.name AS manufacturer
                                     FROM filament_profiles fp
                                     JOIN materials m ON m.id=fp.material_id
                                     JOIN manufacturers mf ON mf.id=fp.manufacturer_id""").fetchall()
                PRESERVED_PROFILE_IDS = {
                    make_filament_key(r["material"], r["manufacturer"], r["line"] or r["commercial_name"]): r["id"]
                    for r in rows
                }
            finally:
                old.close()
            info(f"IDs preservados por filament_key: {len(PRESERVED_PROFILE_IDS)} perfis")
    ...
    CREATE TABLE filament_profiles (
        ...
        filament_key TEXT NOT NULL UNIQUE,
